...
describe BinaryGame do
  ...
  describe '#verify_input' do
    ...
    context 'when given a valid input as argument' do
      it 'returns valid input' do
        ...
        valid_input = 5
        expect(game_verify).to receive(:verify_input).and_return(valid_input).once
        game_verify.verify_input(min, max, valid_input)
      end
    end
  ...
  describe '#display_binary_search' do
    ...
    context 'when game_over? is false five times' do
      before { allow(search_display).to receive(:game_over?).and_return(false, false, false, true) }

      it 'calls display_turn_order five times' do
        expect(game_display).to receive(:display_turn_order).with(search_display).exactly(3).times
        game_display.display_binary_search(search_display)
      end
    end
  end
  ...
  describe '#display_turn_order' do
    ...
    let(:search_order) { double(BinarySearch) }
    ...
    it 'increases guess_count by one' do
      expect { order_game.display_turn_order(search_order) }.to change { order_game.instance_variable_get(:@guess_count) }.by(1)
    end

    # Method with Outgoing Command -> Test that a message is sent
    it 'sends make_guess' do
      expect(search_order).to receive(:make_guess).once
      order_game.display_turn_order(search_order)
    end
