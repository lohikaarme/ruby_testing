...
describe FindNumber do
  ...
  describe '#make_guess' do
    ...
    context 'when min is 5 and max is 9' do
      subject(:game_5_9) { described_class.new(5, 9, number_guessing) }
      it 'returns 7' do
        solution = game_5_9.make_guess
        expect(solution).to eq(7)
      end
    end

    context 'when min is 8 and max is 9' do
      subject(:game_8_9) { described_class.new(8, 9, number_guessing) }
    it 'returns 8' do
        solution = game_8_9.make_guess
        expect(solution).to eq(8)
      end
    end

    context 'when min is 0 and max is 3' do
      subject(:game_0_3) { described_class.new(0, 3, number_guessing) }
      it 'returns 1' do
        solution = game_0_3.make_guess
        expect(solution).to eq(1)
      end
    end

    context 'when min and max both equal 3' do
      subject(:game_3_3) { described_class.new(3, 3, number_guessing) }
      it 'returns 3' do
        solution = game_3_3.make_guess
        expect(solution).to eq(3)
      end
    end
  end
  ...
  describe '#game_over?' do
    context 'when guess and random_number are equal' do
      ...
      let(:rnd_number) { double('random_number', value: 8) }
      subject(:game_end) { described_class.new(0, 9, rnd_number, 8) }
    ...
    context 'when guess and random_number are not equal' do
      let(:rnd_number) { double('random_number', value: 8) }
      subject(:game_on) { described_class.new(0, 9, rnd_number, 5) }
      it 'is not game over' do
        expect(game_on).not_to be_game_over
      end
    end
  end
  ...
  describe '#update_range' do
    ...
    context 'when the guess is 7, min=5, and max=8' do
      ...
      it 'updates min to the same value as max' do
        min = edge_guess_game.min
        max = edge_guess_game.max
